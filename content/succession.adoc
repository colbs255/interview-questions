...
Whenever a child is added to structure, we can simply append the child to this list.
...
When removing person A, their children need to be promoted up a level and we need to ensure their children have higher priority than A's siblings.
...
To make removals easier, we instantiate our structure with a dummy root node where the first child of the root node be the CEO.
